...
void BmpFile:: rearrangeY(){
	for(int y = 0; y < height; y++) {
		for(int x = 0; x < width; x++) {
			int position = width*y + x;
			if(max_Y < Y[position])max_Y = Y[position];
			if(min_Y > Y[position])min_Y = Y[position];
		}
	}
	for(int y = 0; y < height; y++) {
		for(int x = 0; x < width; x++) {
...
void BmpFile::YUV2RGB(){
	for(int y = 0; y < height; y++) {
		for(int x = 0; x < width; x++) {
			int position = width*y + x;
			//printf("--\n");
			R[position] = this->clip_value(1.0*Y[position] - 0.00093*U[position] + 1.401687*(V[position]-128));
			G[position] = this->clip_value(1.0*Y[position] - 0.3437*(U[position]-128) - 0.71417*(V[position]-128));
			B[position] = this->clip_value(1.0*Y[position] + 1.77216*(U[position]-128) + 0.00099*(V[position]-128));
		}
	}	
}
...
void BmpFile::convertToGray(){
	...
	for(int y = 0; y < height; y++) {
		for(int x = 0; x < width; x++) {
			int position = width*y + x;
			new_color_data[(row_size*y + 3*x) + 2] = this->clip_value(Y[position]);
			new_color_data[(row_size*y + 3*x) + 1] = this->clip_value(Y[position]);
			new_color_data[(row_size*y + 3*x) + 0] = this->clip_value(Y[position]);
...
void BmpFile::bilateral_filter(double s, double r, int len){
	int l = len/2;
	double value = 0;
		...
		for (int j = 0; j < width; ++j)
		{
			...
			for (int m = 0; m < len; ++m)
			{
				for (int n = 0; n < len; ++n)
				{
					if(j-l+n>=0 && j-l+n<width && i-l+m>=0 && i-l+m < height){
						int new_position = (i-l+m)*width + j-l+n;
						double gs = exp( -1.0*((l-m)*(l-m) + (l-n)*(l-n))/s_2 );
						double r_gr = exp( -1.0*(R[new_position]-R[position])*(R[new_position]-R[position])/r_2 );
						double g_gr = exp( -1.0*(G[new_position]-G[position])*(G[new_position]-G[position])/r_2 );
						double b_gr = exp( -1.0*(B[new_position]-B[position])*(B[new_position]-B[position])/r_2 );
						r_M += gs*r_gr;
						g_M += gs*g_gr;
						b_M += gs*b_gr;
						r_value += gs*r_gr*R[new_position];
						g_value += gs*g_gr*G[new_position];
						b_value += gs*b_gr*B[new_position];
					}
				}
			}
			R[position] = this->clip_value(r_value/r_M);
			G[position] = this->clip_value(g_value/g_M);
			B[position] = this->clip_value(b_value/b_M);
		}
	}
	this->color2data();
}
